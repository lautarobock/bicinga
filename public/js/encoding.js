function entityToHtml(string) {
	for ( var i in entity_table) {
		if (i != 38) {
			string = string.replace(new RegExp(entity_table[i], "g"), String
					.fromCharCode(i));
		}
	}
//	string = string.replace(new RegExp("&#(x?)(\\d+);", "g"), String
//			.fromCharCode(((p1 == 'x') ? parseInt(p2, 16) : p2)));
//	string = string.replace(new RegExp(entity_table[38], "g"), String
//			.fromCharCode(38));
	return string;
}
